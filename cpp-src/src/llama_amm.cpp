...
// Apply a trade specified by (i, j, dx) — figures out dy from calc_swap_out
// and writes the resulting band deltas. Used for synthesized arb trades.
...
// Bisect on trade size to bring AMM marginal price ≈ target_p.
// Directional logic: if AMM marginal > target_p, arb sends crvUSD in (pump)
// which drains active band's y → active_band drops → marginal price drops.
// Conversely if AMM marginal < target_p, arb sends CRV in (i=1, j=0) which
// raises marginal price. We bisect on dx up to an upper bound derived from
// the sum of the token being sent in that's already sitting in the bands.
SynthTrade arb_to_target_price(const LlammaImmutables& im, LlammaState& s, u256 target_p) {
    SynthTrade out;
    if (target_p == 0) return out;
    // Bypass LLAMMA's per-tx anti-manipulation guardrail (limit_p_o clamps
    // p_o to move at most 1.25× per PREV_P_O_DELAY=120s window). Real arbs
    // trip that guardrail on each of their many txs; but we synthesize the
    // *aggregate* end-of-block equilibrium in a single call, and the guardrail
    // would prevent us from reaching target within one call when the target
    // shifts fast (crash-scenario, arb-target=real-market-per-block etc.).
    // Since our synthetic trade represents a collective off-chain force, not
    // a single user-tx that could manipulate, disabling the guardrail here
    // is correct. Push prev_p_o_time far enough back that dt = 0 (no clamp)
    // and seed old_p_o at the target so limit_p_o becomes an identity.
    if (s.block_timestamp > u256(120)) {
    ...
    u256 cur = get_p(im, s);
    static const u256 ONE = ONE_1E18();
    // 0.02% tolerance (2e14) — well below any per-block schedule granularity.
    u256 tol = ONE / 5000;
    u256 diff = (cur > target_p) ? (cur - target_p) : (target_p - cur);
    if (diff < tol) return out;
    // Direction (verified: cur=$0.62 + send 1.71M CRV → pmax=$0.17):
    //   send crvUSD in (i=0, j=1) → drains y from bands → active_band moves
    //     toward the crvUSD-heavy side → marginal RISES toward higher band.
    //   send CRV in    (i=1, j=0) → fills bands with y → active_band moves
    //     toward the y-heavy side → marginal FALLS toward lower band.
    // Therefore:
    //   need cur ↑ (cur < target)  → pump = true  → i=0, j=1  (crvUSD in)
    //   need cur ↓ (cur > target)  → pump = false → i=1, j=0  (CRV in)
    bool pump = (cur < target_p);
    uint64_t i = pump ? 0 : 1;
    uint64_t j = pump ? 1 : 0;
    // Upper bound: 4× total of the counter-token across all bands (arb never
    // needs more than that to drain everything the AMM has of the other side).
    u256 total_other = 0;
    for (auto& kv : s.bands) {
        total_other += pump ? kv.second.y : kv.second.x;
    }
    if (total_other == 0) return out;
    // Convert to the input-token scale via target price (rough heuristic).
    // For pump (crvUSD in, CRV out): dx in crvUSD ≈ target · dy(CRV). Cap at 4× that.
    // For ~pump (CRV in, crvUSD out): dx in CRV ≈ dy(crvUSD) / target. Cap at 4× that.
    u256 upper;
    if (pump) upper = (target_p * total_other) / ONE * u256(4) / im.COLLATERAL_PRECISION;
    else      upper = (total_other * ONE) / target_p * u256(4) / im.BORROWED_PRECISION;
    if (upper == 0) upper = u256(1000000) * ONE;  // fallback 1e6 tokens

    // PROFIT-SEEKING sizing: maximize mark PnL instead of forcing price=target.
    //   LIQ  (pump): pay dx crvUSD, get dy CRV  → PnL = dy·target − dx
    //   DELIQ:       pay dx CRV,   get dy crvUSD → PnL = dy − dx·target
    // PnL(dx) is concave (marginal execution price worsens monotonically along
    // the band walk, fee included) → ternary search. If the maximum is ≤ 0 the
    // arb REFUSES to trade — no unprofitable "price maintenance" trades. This
    // implicitly never pushes past target (beyond it every extra unit loses).
    const double tgt_d = target_p.convert_to<double>() / 1e18;
    auto eval_pnl = [&](const u256& dx) -> double {
        if (dx == 0) return 0.0;
        LlammaState c = s;
        try { apply_trade_dx(im, c, i, j, dx); } catch (...) { return -1e30; }
        u256 dy = 0;
        for (auto& kv : s.bands) {
            auto it2 = c.bands.find(kv.first);
            if (it2 == c.bands.end()) continue;
            if (pump) { if (kv.second.y > it2->second.y) dy += kv.second.y - it2->second.y; }
            else      { if (kv.second.x > it2->second.x) dy += kv.second.x - it2->second.x; }
        }
        double dxd = dx.convert_to<double>() / 1e18;
        double dyd = dy.convert_to<double>() / 1e18;
        return pump ? (dyd * tgt_d - dxd) : (dyd - dxd * tgt_d);
    };
    u256 lo = 0, hi = upper;
    for (int it = 0; it < 60 && hi - lo > u256(1000); ++it) {
        u256 m1 = lo + (hi - lo) / 3;
        u256 m2 = hi - (hi - lo) / 3;
        if (eval_pnl(m1) < eval_pnl(m2)) lo = m1;
        else                             hi = m2;
    }
    u256 best = (lo + hi) / 2;
    if (best == 0 || eval_pnl(best) <= 0.0) return out;
    // Apply the final trade to real state.
    LlammaState before = s;
    apply_trade_dx(im, s, i, j, best);
    // Report the actual (dx, dy) that landed — dy is inferred from band delta.
    u256 dy = 0;
    for (auto& kv : before.bands) {
        auto it2 = s.bands.find(kv.first);
        if (it2 == s.bands.end()) continue;
        u256 y_before = kv.second.y, y_after = it2->second.y;
        u256 x_before = kv.second.x, x_after = it2->second.x;
        if (pump) { if (y_before > y_after) dy += y_before - y_after; }
        else      { if (x_before > x_after) dy += x_before - x_after; }
    }
    out.i = i; out.j = j; out.dx = best; out.dy = dy;
    return out;
}
