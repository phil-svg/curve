...
struct LlammaState {
    // mutables. V2: no admin_fee / admin_fees_x / admin_fees_y — the V2 AMM
    // no longer siphons a per-swap admin fee. Protocol take happens at the
    // Controller via admin_percentage on interest.
    u256 fee = 0;
    u256 rate = 0;
    u256 rate_time = 0;
...
struct DetailedTrade {
    ...
    u256 last_tick_j = 0;
    // V2: no admin_fee field — all fee revenue folds into the LP invariant.
};
