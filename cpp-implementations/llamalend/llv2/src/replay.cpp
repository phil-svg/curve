...
static void load_snapshot(const std::string& path, LlammaImmutables& im, LlammaState& s) {
    ...
    s.fee            = U(st["fee"]);
    // V2: no admin_fee / admin_fees_x / admin_fees_y in state.
    s.rate           = U(st["rate"]);
...
static void dump_state(const LlammaState& s, const std::string& out_path) {
    json j;
    // V2: no admin_fee / admin_fees_x / admin_fees_y — dropped from state dump.
    j["state"] = {
        {"fee",           s.fee.str()},
        {"rate",          s.rate.str()},
    ...
    for (size_t k = 0; k < events.size() && k < stop_at; ++k) {
        ...
        try {
            if (kind == "TokenExchange") {
                ...
                // Fee for this block was captured by the fetcher (V2 has no
                // SetFee event, so we injected fee() per swap block into the
                // event). Adopt it before applying the swap.
                if (e.contains("fee")) s.fee = U(e["fee"]);
                auto pre_active = s.active_band;
                apply_token_exchange(im, s, i, j, sold, bought);
                // Divergence marker: if pre/post active_band unchanged for a
                // non-zero swap, the exchange did no work → flag for review.
                if (s.active_band == pre_active && (sold > 0 || bought > 0)) {
                    static int _n = 0;
                    if (_n < 20) {
                        std::fprintf(stderr,
                ...
                // V2 layout: (user indexed, collateral, borrowed, debt, n1, n2, liquidation_discount)
                std::string u = e["user"].get<std::string>();
                u256 collat   = U(e["collateral"]);
                u256 borrowed = e.contains("borrowed") ? U(e["borrowed"]) : u256(0);
                u256 debt     = U(e["debt"]);
                i256 n1 = e.contains("n1") ? i256(e["n1"].get<int64_t>()) : i256(0);
                i256 n2 = e.contains("n2") ? i256(e["n2"].get<int64_t>()) : i256(0);
                if (collat == 0 && borrowed == 0 && debt == 0 && n1 == 0 && n2 == 0) {
                    s.users.erase(u);
                } else {
                    UserTicks& ut = s.users[u];
                    ut.collateral = collat;
                    ut.stablecoin = borrowed;   // V2 gives us `x` directly.
                    ut.debt       = debt;
                    ut.N          = u256(n2 - n1 + 1);
                }
            }
            // All the other V2 events we captured (Borrow / Repay /
            // RemoveCollateral / Liquidate / SetExtraHealth / CollectFees /
            // Vault* / Approval etc.) are informational; UserState already
            // reflects the post-op user state, so replay ignores them.
