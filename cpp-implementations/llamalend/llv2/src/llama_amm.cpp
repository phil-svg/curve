...
DetailedTrade calc_swap_out(const LlammaImmutables& im, const LlammaState& s,
    ...
    // V2: no admin_fee siphon at the AMM.
        ...
        if (pump) {
            if (y != 0 && g != 0) {
                ...
                if (dx >= in_amount_left) {
                    ...
                    out.ticks_in[j] = x;   // V2: no admin siphon
                    out.in_amount = in_amount;
                    break;
                } else {
                    dx = std::max<u256>(dx, 1);
                    in_amount_left -= dx;
                    out.ticks_in[j] = x + dx;   // V2: no admin siphon
                    out.in_amount += dx;
                    out.out_amount += y;
                }
            }
            ...
            if (x != 0 && f != 0) {
                ...
                if (dy >= in_amount_left) {
                    ...
                    out.ticks_in[j] = y;   // V2: no admin siphon
                    out.in_amount = in_amount;
                    break;
                } else {
                    dy = std::max<u256>(dy, 1);
                    in_amount_left -= dy;
                    out.ticks_in[j] = y + dy;   // V2: no admin siphon
                    out.in_amount += dy;
                    out.out_amount += x;
                }
            }
...
void apply_token_exchange(const LlammaImmutables& im, LlammaState& s,
    ...
    // V2: no admin_fees_x / _y bookkeeping — swap fees fold into the invariant.
...
std::pair<u256, u256> apply_withdraw(const LlammaImmutables& im, LlammaState& s,
    ...
    for (i256 n = n_lo; ; n += 1) {
        ...
        y -= dy;
        if (new_shares == 0) {
            // V2: leave dust in the AMM (there's no admin_fees_x / _y sink).
            // Mirrors the Vyper source comment: "leave dust in the AMM".
            x = 0; y = 0;
        }
