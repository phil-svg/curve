...
from sweep_ref_table import _disc_coeff, _load_kl_rows  # noqa: E402
...
def ensure_cache_klines(kl: Path, texp: int) -> tuple[Path, Path, dict]:
    """Market/oracle for a packed USD-basis kline series (fetch_binance_
    klines / build_nav_klines output): market = the rows themselves (t in
    seconds), oracle = the author's build_oracle with aggregate 1.0 — EMA of
    the candle midpoint, 2^(-dt/hl) decay, seeded at the first open."""
    binf = Path(str(kl) + ".bin")
    sz = binf.stat().st_size if binf.exists() else kl.stat().st_size
    key = hashlib.sha1(f"{kl}|{sz}|{texp}".encode()).hexdigest()[:12]
    CACHE.mkdir(parents=True, exist_ok=True)
    mkt_f, orc_f = CACHE / f"market_{key}.json", CACHE / f"oracle_{key}.json"
    meta_f = CACHE / f"market_{key}.meta.json"
    if not (mkt_f.exists() and orc_f.exists() and meta_f.exists()):
        rows = _load_kl_rows(kl)
        market = [[int(r[0]) // 1000, float(r[1]), float(r[2]), float(r[3]),
                   float(r[4]), 0.0] for r in rows]
        ema = market[0][1]
        prev = market[0][0]
        oracle = []
        for t, _o, h, l, _c, _v in market:
            decay = 2 ** (-(t - prev) / texp) if texp else 0.0
            ema = ema * decay + (h + l) / 2 * (1 - decay)
            oracle.append(ema)
            prev = t
        mkt_f.write_text(json.dumps(market))
        orc_f.write_text(json.dumps(oracle))
        meta_f.write_text(json.dumps({"n": len(market),
                                      "from": market[0][0],
                                      "to": market[-1][0]}))
    return mkt_f, orc_f, json.loads(meta_f.read_text())
...
def main() -> None:
    ...
    ap.add_argument("--klines-file", type=Path, default=None,
                    help="packed USD-basis kline series (needs a sibling "
                         ".meta.json) instead of the author's ZCHF data")
    ...
    if args.klines_file:
        ...
        mkt_f, orc_f, meta = ensure_cache_klines(args.klines_file,
                                                 int(args.texp))
        collateral = kl_meta["symbol"]
        pool_name = kl_meta.get("pool_name") or kl_meta.get("source", "")
    else:
